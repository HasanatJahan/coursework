...
year_counts <- read.table("year_counts.tsv")
...
# filtering out the years to contain years starting from 1800
year_counts <- year_counts %>% filter(year >= 1800)
total_counts <- total_counts %>% filter(year >1800)
...
> Recreate the main part of figure 3a of Michel et al. (2011). To recreate this figure, you will need two files: the one you downloaded in part (a) and the “total counts” file, which you can use to convert the raw counts into proportions. Note that the total counts file has a structure that may make it a bit hard to read in. 
...
joined_years_and_totals <- 
  joined_years_and_totals %>%
  rename(
    book_count_year = book_count.y,
    book_count_match = book_count.x
  )
...
Use the `percent` function from the `scales` package for a readable y axis. Each term should have a different color, it's nice if these match the original paper but not strictly necessary.
...
joined_years_and_totals %>% 
  mutate(frequency = volume/total_volume, 
         term_as_factor = as.character(term))%>% 
  filter(term == c(1883,1910, 1950)) %>%
  ggplot() + 
  geom_line(
    aes(x=year, 
        y=frequency,
        color=term_as_factor)) +
...
Yes, it does produce similar results. There is a peak at the year of term and then it goes down as the years go by to go up again. 
...
Add your screenshot for Part C below this line using the `![](Ngram_viewer_screenshot.png)` syntax and comment on similarities / differences.
...
plot_D <- 
joined_years_and_totals %>% 
  mutate(term_as_factor = as.character(term))%>% 
  filter(term == c(1883,1910, 1950)) %>%
  ggplot() + 
  geom_line(
    aes(x=year, 
...
colnames(joined_years_and_totals)

joined_years_and_totals %>% 
  select(year, total_volume) %>% 
  ggplot() +
  geom_line(aes(x=year, y=total_volume)) + scale_y_continuous(label = comma)
...
I think that I would re-evaluate Michel's claims. As the frequency the mentions does decrease over time, we should account that the total volume of books per year does increase by a lot, exponentially even. By the raw counts data, for 1883 and 1910, while there is an inital dip of the mentions, it actually incrseses beyond it's first peak going after 2000s. For 1950, while there is an initial dip in the first 50 years after it's peak like for 1910 and 1883, it shows the trend of increase. 

# Part F

> Now, using the proportion of mentions, replicate the inset of figure 3a. That is, for each year between 1875 and 1975, calculate the half-life of that year. The half-life is defined to be the number of years that pass before the proportion of mentions reaches half its peak value. 
...
joined_years_and_totals %>% 
  mutate(frequency = volume/total_volume, 
         term_as_factor = as.character(term)) %>%
  select(term, year, frequency) %>% 
  filter(year > 1800 & year <= 2000) %>%
  group_by(term) %>% 
  arrange(term, desc(frequency))  %>%
...
half_life_df <- 
  ...
  group_by(term) %>% 
  arrange(term, desc(frequency)) %>%
  mutate(max_frequency = quantile(frequency, 1.0)) %>%
  mutate(percentage = floor(frequency/max_frequency * 100)) %>%
  ...
  #mutate(frequency = volume/total_volume, 
   #      term_as_factor = as.character(term)) %>%
  #select(term, year, frequency) %>%
  #filter(y > 1800 & year <= 2000) %>%
  #group_by(term) %>% 
#  mutate(half_quant = quantile(volume, 0.50)) %>% 
  #filter(volume == half_quant) %>%
...
years <- c(1883, 1910, 1950)

half_life_df %>%
  mutate(term_as_factor= as.character(term)) %>%
  filter(year>= 1850 & year <=2012 & year > term) %>% 
  filter(half_life >= 0) %>%
...
It seems that years around 1810, around 1835 and the years around 1850 seem like outliers, and they were forgotten particularly quickly. 
As according to US history, 1810 was not a historically significant year, around 1825 there was not a lot of political change, same for 1850 and 1852. 
It seems that there might not have been a lot of impactful political events during these times and the mention of these years in books might be linked to the political significance of the years. 
