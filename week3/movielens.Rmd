...
Nintypct_satisfaction <- 
  combined_df %>%
  summarize(quants = quantile(movie_rank, probls = 0.9)) %>%
  arrange(quants) %>% 
  group_by(quants) %>%
  summarise(quant_count = n()) %>%
  mutate(cumsum_quant = cumsum(quant_count))
#%>%
#  ggplot(aes(x=quants, y = cumsum_quant)) +
#  geom_line()

#selected_100pct <- hundred_percent_satisfaction %>% select(movie_rank, cumsum_ranking_count, movie_rank)
#selected_100pct

#selected_90pct <- Nintypct_satisfaction %>% select(quants, cumsum_quant, movie_rank)
#selected_90pct

#joined_df <- selected_100pct %>% full_join(selected_90pct, by="movie_rank")
